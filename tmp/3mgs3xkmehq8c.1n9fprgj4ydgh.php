...
<div class="container">
    ...
    <form action="<?= ($BASE) ?>/admin/sprints/new" method="post" class="form-horizontal" autocomplete="off">
        ...
        <div class="form-group">
            ...
            <div class="col-md-10">
                <input class="form-control input-sm" id="name" type="text" name="name" value="<?= ($this->esc(empty($POST) ? "" : $POST['name'])) ?>">
            </div>
        </div>
        <div class="form-group">
            <label for="start_date" class="col-md-2 control-label label-sm"><?= ($dict['start_date']) ?></label>
            <div class="col-md-4">
                <input class="form-control input-sm" id="start_date" type="text" name="start_date" value="<?= ($this->esc(empty($POST) ? "" : $POST['start_date'])) ?>" required>
            </div>
        </div>
        <div class="form-group">
            <label for="end_date" class="col-md-2 control-label label-sm"><?= ($dict['end_date']) ?></label>
            <div class="col-md-4">
                <input class="form-control input-sm" id="end_date" type="text" name="end_date" value="<?= ($this->esc(empty($POST) ? "" : $POST['end_date'])) ?>" required>
            </div>
        </div>
        <div class="form-group">
